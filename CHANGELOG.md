...
- **BREAKING — the extension id is now `jira-mirror`, and it installs to
  `.specify/extensions/jira-mirror/`.** The id `jira` is already held in the
  Spec Kit community catalog by another extension, so this one could not be
  published under it (Constitution XII: the catalog id must be verified
  available before publication). The id is also the ownership key the host
  writes into every `.specify/extensions.yml` entry it registers for us, so the
  rename moves the install directory and the registry key together.

  **Migration — remove the old installation before adding the new one:**
  ...
  Removing first is not optional housekeeping. The two ids are distinct owners
  as far as the registry is concerned: an entry left behind under
  `extension: jira` is neither recognised as ours nor classified as an
  ownerless leftover, so the bridge will neither report nor purge it, and it
  names the same command the new entry names. Skipping the removal leaves both
  entries in place and every lifecycle event fires the reconcile twice. The
  second run is idempotent and writes nothing to Jira (Constitution II), so the
  cost is a duplicated prompt and a wasted run rather than damage — but it will
  not clear up on its own.

  Nothing else about the extension changes. The four commands keep their names
  (`/speckit.jira.config`, `/speckit.jira.feature`, `/speckit.jira.reconcile`,
  `/speckit.jira.seed`), your configuration under `.specify/jira/` is untouched
  — it has never lived inside the extension folder (Constitution V) — and the
  environment variables and credential resolution are unchanged.

- The catalog display name is now **Jira Mirror**, and the manifest
  description states the differentiator the old one left implicit: both project
  styles, configurable workflows and hierarchies, multi-project.

- **The repository moved to
  [`Fyloss/spec-kit-jira-mirror`](https://github.com/Fyloss/spec-kit-jira-mirror).**
  Every address in this repository — the manifest's `repository` and `homepage`,
  the install commands, the remedies the bridge prints — now names it. GitHub
  redirects the old address, so an existing clone, bookmark, or pinned install
  URL keeps working; nothing is required of you. The published release **asset**
  is still `spec-kit-jira.zip` (and `spec-kit-jira-<version>.zip` when pinned) —
  the file name is independent of the repository name and did not change.

### Added

- README now documents how the bridge resolves issue types, hierarchy and
  workflow against a team-managed versus a company-managed project.
