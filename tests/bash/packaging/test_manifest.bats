...
# `speckit.jira-mirror.seed` MUST be declared in `extension.yml` under
...
@test "speckit.jira-mirror.seed is declared under provides.commands" {
  [[ "$(_provides_block)" == *"name: speckit.jira-mirror.seed"* ]]
}

@test "speckit.jira-mirror.seed's file points at commands/speckit.jira-mirror.seed.md" {
  local block
  block="$(_provides_block)"
  [[ "${block}" == *"speckit.jira-mirror.seed.md"* ]]
}

@test "speckit.jira-mirror.seed is bound to no hook event" {
  local hooks
  hooks="$(_hooks_block)"
  [[ "${hooks}" != *"speckit.jira-mirror.seed"* ]]
}

@test "the command definition file exists and is shipped content" {
  [ -f "${ROOT}/commands/speckit.jira-mirror.seed.md" ]
}
