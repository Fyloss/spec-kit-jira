...
Describe 'ConvertTo-JiraMarkdownBlockList — Part B block segmentation' {
    It 'B1 — a fenced code block is verbatim, no inline tokenization (FR-007)' {
        $doc = "``````bash`necho **not bold**`n``````"
        $got = Get-Norm (ConvertTo-JiraMarkdownBlockList -Text $doc)
    ...
    It 'B2 — an ATX heading carries its level and tokenized text' {
        $got = Get-Norm (ConvertTo-JiraMarkdownBlockList -Text '### Design **Notes**')
    ...
    It 'B3 — bullet items accumulate into one bullet_list, nesting flattened' {
        ...
        $out = ConvertTo-JiraMarkdownBlockList -Text $doc | ConvertFrom-Json -Depth 20
    ...
    It "B5 — a blockquote's prefix is stripped and the remainder re-segments" {
        $got = Get-Norm (ConvertTo-JiraMarkdownBlockList -Text '> a quoted paragraph')
        $exp = Get-Norm '[{"type":"paragraph","spans":[{"text":"a quoted paragraph","marks":[]}]}]'
        $got | Should -Be $exp
    }

    It 'B5 — a blockquoted heading still segments as a heading' {
        $got = Get-Norm (ConvertTo-JiraMarkdownBlockList -Text '> ## Quoted Heading')
...
Describe 'ConvertTo-JiraMarkdownBlockList — B9 selection cap worked example (data-model.md §4)' {
...
Describe 'ConvertTo-JiraMarkdownBlockList — T086 CRLF equals LF (FR-015, spec Edge Cases)' {
    It 'a CRLF document yields the same blocks as the LF original' {
        $lf = Get-Norm (ConvertTo-JiraMarkdownBlockList -Text "## Overview`nSome intro prose.`n- item a`n- item b")
        $crlf = Get-Norm (ConvertTo-JiraMarkdownBlockList -Text "## Overview`r`nSome intro prose.`r`n- item a`r`n- item b")
        $lf | Should -Be $crlf
    }
}
