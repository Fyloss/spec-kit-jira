...
Describe 'ConvertTo-JiraMarkdownInlineSpanList — Part E worked examples' {
    It 'E1 — **FR-012** applies: one bold span' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '**FR-012** applies')
        $exp = Get-Norm '[{"text":"FR-012","marks":[{"kind":"bold"}]},{"text":" applies","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'E2 — code span protects its interior and gets monospace' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text 'run `reconcile --dry-run`')
    ...
    It 'E3 — a valid link renders as a link mark, target hidden' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text 'see [guide](https://ex.invalid/s)')
    ...
    It 'E4 — an unsafe target degrades to label (target), no link mark (FR-006)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text 'see [guide](../local.md)')
        $exp = Get-Norm '[{"text":"see guide (../local.md)","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'E5 — italic (star and underscore) and strikethrough' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '*a*, _b_, ~~c~~')
    ...
    It 'E6 — backslash-escaped asterisks stay literal (C1)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '\*not bold\*')
        $exp = Get-Norm '[{"text":"*not bold*","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'E7 — 2 * 3 * 4 stays literal (C9.1: space follows the opener)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '2 * 3 * 4')
        $exp = Get-Norm '[{"text":"2 * 3 * 4","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'E8 — parse_description_blocks survives with zero marks (C9.3)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text 'parse_description_blocks')
        $exp = Get-Norm '[{"text":"parse_description_blocks","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'E9 — a code span nested in bold keeps both marks' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '**bold with `code` inside**')
    ...
    It 'E10 — an unclosed bold delimiter stays fully literal (C9.4)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '**unclosed')
    ...
    It 'E12 — an image renders as its alt text, no mark (FR-010)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '![diagram](x.png)')
        $exp = Get-Norm '[{"text":"diagram","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'E13 — an autolink renders the URL as a link span (C3)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '<https://ex.invalid>')
    ...
    It 'E14 — a raw HTML tag is discarded, inner text kept (C10)' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '<b>text</b>')
        $exp = Get-Norm '[{"text":"text","marks":[]}]'
        $got | Should -Be $exp
    }
}

Describe 'ConvertTo-JiraMarkdownInlineSpanList — C9 delimiter rules' {
    It 'C9.1 — opener must be followed by non-whitespace' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '* not emphasis *')
        $exp = Get-Norm '[{"text":"* not emphasis *","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'C9.2 — the nearest valid closer wins' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '*a* b *c*')
    ...
    It 'C9.3 — underscore inside an identifier never opens emphasis' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text 'customfield_10011')
        $exp = Get-Norm '[{"text":"customfield_10011","marks":[]}]'
        $got | Should -Be $exp
    }

    It 'C9.3 — a standalone underscore-wrapped word still emphasises' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text 'a _word_ here')
    ...
    It 'C9.4 — no closer means every delimiter char is literal' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text 'a _b')
    ...
    It 'C9.6 — depth cap of 8 bounds pathological nesting deterministically' {
        ...
        { ConvertTo-JiraMarkdownInlineSpanList -Text $input } | Should -Not -Throw
    }
}

Describe 'ConvertTo-JiraMarkdownInlineSpanList — D1-D3 emission invariants' {
    It 'D1 — adjacent equal-mark spans merge into one' {
        $out = ConvertTo-JiraMarkdownInlineSpanList -Text '[a](https://ex.invalid)[b](https://ex.invalid)' | ConvertFrom-Json -Depth 20
    ...
    It 'D2 — marks is always present and sorted alphabetically by kind' {
        $out = ConvertTo-JiraMarkdownInlineSpanList -Text '**_~~x~~_**' | ConvertFrom-Json -Depth 20
        ($out[0].marks | ForEach-Object { $_.kind }) -join ',' | Should -Be 'bold,italic,strikethrough'
    }

    It 'D3 — an empty inline collapses to []' {
        ConvertTo-JiraMarkdownInlineSpanList -Text '' | Should -Be '[]'
    }
}

Describe 'ConvertTo-JiraMarkdownInlineSpanList — T085 non-ASCII inside a formatted span (spec Edge Cases)' {
    It 'accented characters, CJK and emoji inside a bold span survive byte-for-byte' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '**café 日本語 🎉**')
    ...
    It 'non-ASCII inside a link label survives byte-for-byte' {
        $got = Get-Norm (ConvertTo-JiraMarkdownInlineSpanList -Text '[café 日本語 🎉](https://ex.invalid/s)')
