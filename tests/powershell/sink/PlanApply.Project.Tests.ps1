...
BeforeAll {
    ...
    function New-Doc {
        ...
        $doc = [ordered]@{
            ...
            epic           = [ordered]@{ strategy = 'per_repo'; title = 'E'; description = [ordered]@{ blocks = @([ordered]@{ type = 'paragraph'; text = 'e' }) } }
...
Describe 'plan_writes declares the resolved project (US1)' {
    It 'every POST body carries a non-empty fields.project.key equal to routing.project_key (FR-022, FR-023)' {
        ...
        $out = @(Get-JiraPlanWriteSet -NeutralDocJson $doc -PlanContextJson $ctx | ConvertFrom-Json)
        $out[0].method | Should -Be 'POST'
        $out[0].body.fields.project.key | Should -Be 'COMP'
    }
    ...
    It 'the base fields come from Get-JiraCreateFieldsBase, unchanged (FR-025, SC-010)' {
        ...
        $out = @(Get-JiraPlanWriteSet -NeutralDocJson $doc -PlanContextJson $ctx | ConvertFrom-Json)
        $out[0].body.fields.project | ConvertTo-Json -Compress | Should -Be ($base.project | ConvertTo-Json -Compress)
        $out[0].body.fields.issuetype | ConvertTo-Json -Compress | Should -Be ($base.issuetype | ConvertTo-Json -Compress)
        $out[0].body.fields.summary | Should -Be $base.summary
    }
}
