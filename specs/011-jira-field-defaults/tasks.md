...
MPP - [X] T001 [P] Extend the mock's `createmeta` responses to carry `allowedValues`, `schema.type`, and a
      non-defaultable field shape, in `tests/conformance/mock-jira/mock-server.ps1`
- [X] T002 [P] Mirror the same `createmeta` shape in the Bash-side transport shim in
      `tests/conformance/mock-jira/curl-shim.sh`, so both ports see identical bytes
- [X] T003 Create the conformance fixture `tests/conformance/fixtures/repo-with-field-defaults/` — a
...
- [X] T004 Add the mock configuration entry for that fixture's project in
...
- [X] T005 [P] Write the failing test for defaultable-field extraction in
...
- [X] T006 [P] Write the PowerShell twin of T005 in `tests/powershell/sink/Discovery.Defaultable.Tests.ps1`.
      Observe it FAIL.
- [X] T007 Implement `_disc_defaultable_fields` beside the existing `_disc_required_fields` in
...
- [X] T008 Implement the twin in `scripts/powershell/sink/jira/Discovery.psm1`, byte-identical output.

### Persistence — the machine-owned layer

- [X] T009 [P] Write the failing test for binding persistence in
...
- [X] T010 [P] Write the PowerShell twin in `tests/powershell/lib/Config.BindingShape.Tests.ps1`.
      Observe it FAIL.
- [X] T011 Carry `defaultable_fields` through `config_resolved_ids_for` in
      `scripts/bash/commands/config.sh`, omitted rather than emitted empty — the same treatment
      `required_fields` already gets.
- [X] T012 Implement the twin in `scripts/powershell/commands/Config.psm1`.

### Reading the team's answers — the committable layer

- [X] T013 [P] Write the failing test for reading and validating `field_defaults` in
...
- [X] T014 [P] Write the PowerShell twin in `tests/powershell/lib/Config.FieldDefaults.Tests.ps1`.
      Observe it FAIL.
- [X] T015 Add `field_defaults` to the top-level key allowlist and the schema checks in
      `_cfg_schema_errors`, plus a reader, in `scripts/bash/lib/config.sh`.
- [X] T016 Implement the twin in `scripts/powershell/lib/Config.psm1`.

### CLI surface

- [X] T017 [P] Write the failing test for the three new flags in `tests/bash/lib/test_cli.bats`:
...
- [X] T018 [P] Write the PowerShell twin in `tests/powershell/lib/Cli.Tests.ps1`. Observe it FAIL.
- [X] T019 Implement the three flags in `scripts/bash/lib/cli.sh`, following the existing
      `--issue-type` parsing shape. A value may contain `=`; split on the first three separators only.
- [X] T020 Implement the twin in `scripts/powershell/lib/Cli.psm1`.

### Satisfiability — one predicate, both gates (research R5)

- [X] T021 [P] Write the failing test in `tests/bash/sink/test_hierarchy.bats`: a required field with a
      ...
      Observe it FAIL.
- [X] T022 [P] Write the PowerShell twin in `tests/powershell/sink/Hierarchy.Tests.ps1`. Observe it FAIL.
- [X] T023 Give `hierarchy_unsatisfiable_fields` a recorded-defaults input and thread it through
...
- [X] T024 Implement the twin in `scripts/powershell/sink/jira/Hierarchy.psm1`.

### Resolution and payload merge (research R2)

- [X] T025 [P] Write the failing test for label→id resolution in
...
- [X] T026 [P] Write the PowerShell twin in `tests/powershell/sink/PlanApply.Defaults.Tests.ps1`.
      Observe it FAIL.
- [X] T027 Write the failing test proving no default can reach an update, in the same file as T025,
...
- [X] T028 Write the PowerShell twin in `tests/powershell/sink/PlanApply.Defaults.Tests.ps1`. Observe
      it FAIL. Not `[P]`: same file as T026.
- [X] T029 [P] Write the failing test for the payload merge in `tests/bash/sink/test_ticket.bats`:
...
- [X] T030 [P] Write the PowerShell twin in `tests/powershell/sink/Ticket.Tests.ps1`. Observe it FAIL.
- [X] T029a Write the failing privacy-guard test in `tests/bash/sink/test_ticket.bats` (FR-024,
...
- [X] T030a Write the PowerShell twin in `tests/powershell/sink/Ticket.Tests.ps1`. Observe it FAIL.
      Not `[P]`: same file as T030.
- [X] T031 Implement the merge in `jira_create_fields_base` in `scripts/bash/sink/jira/ticket.sh`, so
      both creation paths acquire it from the one builder they already share.
- [X] T032 Implement the twin in `scripts/powershell/sink/jira/Ticket.psm1`. Import the config
...
- [X] T033 Resolve the defaults into the plan context and pass them to the create branch in
      ...
      T028 green.
- [X] T034 Implement the twin in `scripts/powershell/sink/jira/PlanApply.psm1`.
...
- [X] T035 [P] [US1] Write the failing conformance scenario
...
- [X] T036 [P] [US1] Write the failing conformance scenario
...
- [X] T037 [P] [US1] Write the failing scenario
...
- [X] T038 [P] [US1] Write the failing Bash command tests in
...
- [X] T039 [P] [US1] Write the PowerShell twin in
      `tests/powershell/commands/Config.FieldDefaults.Tests.ps1`. Observe it FAIL.
- [X] T040 [US1] Write the failing test for the managed region write in
      ...
      file as T038.
- [X] T041 [US1] Write the PowerShell twin in
      `tests/powershell/commands/Config.FieldDefaults.Tests.ps1`. Observe it FAIL. Not `[P]`: same
      file as T039.
- [X] T041a [US1] Write the failing test for the record-time credential refusal in
...
- [X] T041b [US1] Write the PowerShell twin in
      `tests/powershell/commands/Config.FieldDefaults.Tests.ps1`. Observe it FAIL. Not `[P]`: same
      file as T039.
- [X] T041c [US1] Write the failing carry-forward test in
      ...
      T038.
- [X] T041d [US1] Write the PowerShell twin in
      `tests/powershell/commands/Config.FieldDefaults.Tests.ps1`. Observe it FAIL. Not `[P]`: same
      file as T039.

### Implementation for User Story 1

- [X] T042 [US1] Emit the canonical `field_defaults` YAML block in `scripts/bash/lib/config.sh` from
...
- [X] T043 [US1] Implement the twin in `scripts/powershell/lib/Config.psm1`, byte-identical output.
- [X] T044 [US1] Splice that block into `.specify/jira/config.yml` through the existing
...
- [X] T045 [US1] Implement the twin in `scripts/powershell/commands/Config.psm1` using
      `ManagedSection.psm1` unchanged.
- [X] T046 [US1] Add the field-default questions to the ceremony in `scripts/bash/commands/config.sh`,
...
- [X] T047 [US1] Implement the twin in `scripts/powershell/commands/Config.psm1`.
- [X] T048 [US1] Implement the recording-time refusals of contract §2.4 in
...
- [X] T049 [US1] Implement the twin in `scripts/powershell/commands/Config.psm1`. Turns T041b green.
- [X] T050 [US1] Report orphaned entries and not-yet-consumed entries in the ceremony's run summary
      (FR-008, FR-027) in `scripts/bash/commands/config.sh`, as reports rather than errors.
- [X] T051 [US1] Implement the twin in `scripts/powershell/commands/Config.psm1`.
- [X] T052 [US1] Ensure the degraded path returns before any field-default question or write in
      `scripts/bash/commands/config.sh` (FR-009).
- [X] T053 [US1] Implement the twin in `scripts/powershell/commands/Config.psm1`.
...
- [X] T054 [P] [US2] Write the failing scenario
...
- [X] T055 [P] [US2] Write the failing scenario
      ...
      FAIL.
- [X] T056 [P] [US2] Write the failing scenario
...
- [X] T057 [P] [US2] Write the failing Bash tests in
...
- [X] T058 [P] [US2] Write the PowerShell twin in
      `tests/powershell/commands/Reconcile.FieldDefaults.Tests.ps1`. Observe it FAIL.
- [X] T059 [US2] Write the failing test for summary provenance in
...
- [X] T060 [US2] Write the PowerShell twin in
      `tests/powershell/commands/Reconcile.FieldDefaults.Tests.ps1`. Observe it FAIL. Not `[P]`: same
      file as T058.
- [X] T061 [P] [US2] Write the failing dry-run agreement test in
...
- [X] T062 [P] [US2] Write the PowerShell twin in `tests/powershell/commands/Reconcile.DryRun.Tests.ps1`.
      Observe it FAIL.
- [X] T063 [US2] Write the failing non-blocking test in
      ...
      T057.
- [X] T064 [US2] Write the PowerShell twin in
      `tests/powershell/commands/Reconcile.FieldDefaults.Tests.ps1`. Observe it FAIL. Not `[P]`: same
      file as T058.

### Implementation for User Story 2

- [X] T065 [US2] Stop the run after planning and before any write when contract §3.3's conditions hold,
...
- [X] T066 [US2] Implement the twin in `scripts/powershell/commands/Reconcile.psm1`.
- [X] T067 [US2] Emit the `confirmation-pending` object of `data-model.md` §4 — one per run, each field
      once, with its recorded value and `resume_with` — in `scripts/bash/commands/reconcile.sh`.
- [X] T068 [US2] Implement the twin in `scripts/powershell/commands/Reconcile.psm1`.
- [X] T069 [US2] Apply `--field-value` answers with precedence over recorded defaults, for this run
      only, in `scripts/bash/commands/reconcile.sh` (FR-012).
- [X] T070 [US2] Implement the twin in `scripts/powershell/commands/Reconcile.psm1`.
- [X] T071 [US2] Honour the per-project `ask` switch and `--accept-defaults` in
      `scripts/bash/commands/reconcile.sh` (FR-014, FR-015), recording in the summary which of contract
      §3.4's reasons applied.
- [X] T072 [US2] Implement the twin in `scripts/powershell/commands/Reconcile.psm1`.
- [X] T073 [US2] Add the field/source lines and the `config --field-default …` promotion line to the run
      summary in `scripts/bash/commands/reconcile.sh` (FR-022, FR-021).
- [X] T074 [US2] Implement the twin in `scripts/powershell/commands/Reconcile.psm1`.
...
- [X] T075 [US3] Write the failing Bash tests in
...
- [X] T076 [US3] Write the PowerShell twin in
      `tests/powershell/commands/Reconcile.FieldDefaults.Tests.ps1`. Observe it FAIL. Not `[P]`: same
      file as T058.
- [X] T077 [P] [US3] Write the failing test for a non-defaultable field in
...
- [X] T078 [P] [US3] Write the PowerShell twin in `tests/powershell/sink/Hierarchy.Tests.ps1`.
      Observe it FAIL.
- [X] T079 [US3] Write the failing test for a rejected value in
...
- [X] T080 [US3] Write the PowerShell twin in
      `tests/powershell/commands/Reconcile.FieldDefaults.Tests.ps1`. Observe it FAIL. Not `[P]`: same
      file as T058.
- [X] T081 [P] [US3] Write the failing scenario
      ...
      FAIL.
- [X] T081a [US3] Reconcile the shipped `tests/conformance/scenarios/us3-mandatory-field-refusal.json`
...
- [X] T082 [US3] Extend the refusal message with the per-field remedy command in
      `scripts/bash/sink/jira/hierarchy.sh`, keeping the exit code and the zero-writes guarantee
      exactly as they are.
- [X] T083 [US3] Implement the twin in `scripts/powershell/sink/jira/Hierarchy.psm1`.
- [X] T084 [US3] Report a non-defaultable field with its reason, by label, in
      `scripts/bash/sink/jira/hierarchy.sh` and `scripts/bash/commands/config.sh` (FR-010).
- [X] T085 [US3] Implement the twins in `scripts/powershell/sink/jira/Hierarchy.psm1` and
      `scripts/powershell/commands/Config.psm1`.
- [X] T086 [US3] Translate a Jira field-validation rejection into the human message of FR-019 in
      `scripts/bash/commands/reconcile.sh`, without substituting a value and without retrying.
- [X] T087 [US3] Implement the twin in `scripts/powershell/commands/Reconcile.psm1`.
...
- [X] T088 [P] Write and run the inertness scenario
...
- [X] T089 [P] Document `field_defaults`, the `ask` switch, and the managed region in
      `templates/config.yml.template`, self-documenting enough that a tech lead needs no other page
      (Principle XVI).
- [X] T089a [P] Write the failing agent-doc test for the ceremony in
...
- [X] T089b [P] Write the failing agent-doc test for the reconcile in
...
- [X] T090 [P] Add the new closed questions and the `--field-default` flag, normatively, to
...
- [X] T091 [P] Add the consolidated question, the re-invocation with `--accept-defaults` /
...
- [X] T091a [P] State the unreachable-operator contract of §3.10 in
...
- [X] T092 [P] Document the mechanism in `docs/04-config-ceremony.md` (recording) and
      `docs/05-reconcile-flow.md` (applying), with the Mermaid diagrams those files use.
- [X] T093 [P] Move the field-defaults entry in `docs/VISION.md` from the Part 3 backlog to Part 1,
      per its own Part 4 rule for how an item leaves that document.
- [X] T094 [P] Add the CHANGELOG entry for the MINOR version bump in `CHANGELOG.md`.
- [X] T095 Run `bash tests/conformance/ci-conformance.sh` and resolve every cross-port divergence.
...
- [X] T097 [P] Run `shellcheck $(git ls-files '*.sh')`, `actionlint`, and PSScriptAnalyzer clean.
- [X] T098 Confirm the engine/sink boundary greps stay green: no new Jira identifier in
