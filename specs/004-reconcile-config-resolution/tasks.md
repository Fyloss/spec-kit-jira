...
- [X] T057 Read `config.yml` whenever the creation context is not supplied by an explicit override — extend the config-read condition at `scripts/bash/commands/reconcile.sh:262` and `scripts/powershell/commands/Reconcile.psm1:309` to also require `SPEC_KIT_JIRA_PLAN_CONTEXT` to be unset, so a run that overrides only the project key still resolves `priority_map`; write the failing case first in `tests/bash/commands/test_reconcile_plan_context.bats` and `tests/powershell/commands/Reconcile.PlanContext.Tests.ps1` per FR-008 (partial). Required adding a minimal `config.yml` to the `repo-with-reconcile-legacy` fixture (previously intentionally absent) so the pre-existing suite, which overrides project key and epic strategy but not plan context, keeps reading a valid file instead of hitting the not-configured notice — exactly the contract's documented behaviour change for partial-override runs.
- [X] T058 Add a real (non-`--dry-run`) conformance scenario over the config-resolution path in `tests/conformance/scenarios/` and assert its captured `calls.log` request body is byte-identical to the `us8-reconcile-company-managed` prediction, per SC-006 (missing). Added `us8-reconcile-real.json` and `tests/bash/conformance/test_us8_reconcile_real.bats`, mirroring T070's `test_us6_dry_run.bats` pattern (the mock's `calls.log` records `METHOD path` per call, not the body, so the comparison is the same method+path diff US6 already established as the byte-identical-prediction proof).
- [ ] T059 Close the open T051 gate: obtain the kcov statement percentage for `scripts/bash/commands/reconcile.sh` and `scripts/bash/sink/jira/plan_apply.sh` from a Linux CI run and record it in `specs/004-reconcile-config-resolution/tasks.md`, per Constitution XIII (missing) — **still not runnable in this environment** for the same reason as T051: `tests/coverage/bash-coverage.sh` refuses on macOS. No Linux CI run was available in this session to source a number from; this line stays open until the next Linux CI run reports it.
- [X] T060 Tick the acceptance-summary boxes in `specs/004-reconcile-config-resolution/quickstart.md`, recording the evidence behind each, per tasks: T056 (partial). All twelve boxes ticked with the specific test/scenario evidence for each.
- [X] T061 Exercise the branch-2 mock route — add a scenario or test consuming `tests/conformance/mock-jira/configs/priority-allowed.json` so the `createmetaFields` key at `tests/conformance/mock-jira/mock-server.ps1:61` is covered, per tasks: T003a (partial). Added `us8-reconcile-priority-allowed-discovery.json` (mock `createmetaFields: {COMP: company-allowed}`) and `tests/bash/conformance/test_us8_priority_allowed.bats`, asserting the persisted binding carries only the two allowed priorities and that both ports agree.
- [X] T062 Add the no-environment-variables statement to `README.md`, or point its Install section at the paragraph now in `INSTALL.md:59-64`, per tasks: T054 (partial). README's Install section now states the no-environment-variables property directly and links to INSTALL.md's mirroring step for the override variables.
