...
Appended by `/speckit-converge` after assessing the codebase against `spec.md`, `plan.md`, and `tasks.md`. Baseline at assessment time: `bats tests/bash --recursive` 752 pass / 0 fail; `Invoke-Pester tests/powershell -CI` 506 pass / 0 fail; `us1-recognition-second-run.json` byte-identical across both ports. The gaps below are what the artifacts call for and the tree does not yet contain.

- [X] T064 CRITICAL Add `tests/powershell/sink/Recognition.Tests.ps1` mirroring `tests/bash/sink/test_recognition.bats` — the decision table (bound, `marker-mismatch`, `orphan`, `claimed-by-other`, `duplicate-claim` on two stories and on two keys, a ticket with no marker never adopted), the fault matrix (`401`→exit 3, exhausted `429`→exit 2, network drop→exit 2, `404`→re-created with a notice, each asserting zero creation POSTs), and the diagnostics-catalogue wording check — per Constitution XIII (`missing`)
- [X] T065 CRITICAL Add `tests/powershell/commands/Reconcile.Idempotent.Tests.ps1` mirroring `tests/bash/commands/test_reconcile_idempotent.bats` — the duplicate-creation regression, the unrecorded-identifier and unwritable-`spec.md` (exit 4) run sequence, and the `key-unrecorded` fail-closed window — per FR-025, which requires the regression on both implementations (`missing`)
- [X] T066 CRITICAL Add `tests/powershell/engine/StoryMarker.Tests.ps1` mirroring `tests/bash/engine/test_story_marker.bats` — identifier shape and the `SPEC_KIT_JIRA_ID_SOURCE` seam, the marker grammar's valid/ignored/malformed forms, and the byte-preserving splice including CRLF adoption and the not-opened-when-unchanged rule — per Constitution XIII (`missing`)
- [X] T067 CRITICAL Add `tests/powershell/commands/Reconcile.ZeroChurn.Tests.ps1` mirroring `tests/bash/commands/test_reconcile_zero_churn.bats` — zero POST/PUT on an unchanged re-run with `skipped` equal to the story count, exactly one PUT for a single changed story, byte-identical `spec.md`, and `--dry-run` writing neither Jira nor the file — per Constitution XIII (`missing`)
- [X] T068 CRITICAL Add `tests/powershell/commands/Reconcile.Lifecycle.Tests.ps1` mirroring `tests/bash/commands/test_reconcile_lifecycle.bats` — the drift warning and suppressed content write, the withheld transition on a Flagged ticket, zero `POST …/transitions` in every scenario, and the single-WARNING exit 0 under `SPEC_KIT_JIRA_HOOK_CONTEXT` — per Constitution XIII (`missing`)
- [X] T069 CRITICAL Add `tests/powershell/commands/Reconcile.Durability.Tests.ps1` mirroring `tests/bash/commands/test_reconcile_durability.bats` — recognition with no local run history, a renamed specification folder, and project-scoped recognition — per Constitution XIII (`missing`)
- [X] T070 CRITICAL Add `tests/powershell/engine/Parse.Marker.Tests.ps1` mirroring `tests/bash/engine/test_parse_marker.bats` — the marker line excluded from `Get-JiraParseTitle`, description blocks, acceptance criteria, design, priority, and estimation, and `stories[].local_id` carrying the marker's identifier or empty when absent — per Constitution XIII (`missing`)
- [X] T071 Emit the catalogued `re-routed` notice when a recorded ticket's project differs from the routed project — naming the story, the former key and project, and the new key — in `scripts/bash/sink/jira/recognition.sh` and `scripts/powershell/sink/jira/Recognition.psm1`, with the assertion added to `tests/bash/commands/test_reconcile_durability.bats`; both ports currently push the story into `new` silently, per FR-019, the spec's re-routing edge case, and the diagnostics catalogue of [contracts/recognition-contract.md](./contracts/recognition-contract.md) (`missing`)
- [X] T072 Add the human-origin zero-churn assertion — a ticket of human origin carrying prose above the managed panel receives no write on an unchanged re-run, and its prose is never rewritten — in `tests/bash/commands/test_reconcile_zero_churn.bats` and the new `tests/powershell/commands/Reconcile.ZeroChurn.Tests.ps1`, per FR-014 and US2 acceptance scenario 4 (`missing`)
- [X] T073 Add the conformance scenario `tests/conformance/scenarios/us1-recognition-reorder.json` — reorder and retitle stories between two runs, asserting `created: 0` and that each ticket still holds the content of the story whose marker names it — per plan.md's Project Structure and T024; the behaviour is covered by a bats test but not by the three-OS byte-equality corpus (`missing`)
- [X] T074 Resolve the undeclared configuration surface: `_reconcile_phase_status_map` and `_reconcile_halted_statuses` in `scripts/bash/commands/reconcile.sh` and `scripts/powershell/commands/Reconcile.psm1` read `projects[].phase_status_map` and `projects[].halted_statuses`, which no other module reads and no document describes — either document and validate both keys through the existing config layer, or route them through `config_phase_status_targets` so plan.md's "No new config key in either layer" holds (`contradicts`)
- [X] T075 Document the story marker line, the recognition step, and the `recognised` / `assigned` / `skipped` summary counts in `INSTALL.md`, which carries no mention of any of them — per T057 and Constitution XVI; `README.md` and `commands/speckit.jira.reconcile.md` are already done (`partial`)
