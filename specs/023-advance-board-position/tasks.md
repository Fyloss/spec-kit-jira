...
- [ ] T155 [US9] Apply the decode-once shape of `plan_apply.sh:1032–1076` to the resolution loop in `scripts/bash/sink/jira/transitions.sh` and to the due-set assembly in `scripts/bash/sink/jira/plan_apply.sh` — one structured call decoding the whole set, then a pure shell loop, with the `--slurpfile` temp-file spelling that avoids Linux's 128 KiB per-argument cap (depends on T047) — NOT done: `transitions_load` still spawns two `jq` calls (entry build + `json_canonical`) per due ticket; T150's own B3 test only asserts sub-linear growth against this, not a fixed floor, so it does not force this refactor. Left as the next unit of work
