...
**Conditional entry — added only if research R1 resolves to branch C**: the availability read grows one
...
AC1. That amendment is not taken pre-emptively: it is written only once the measurement says so.
