...
**Purpose**: Close the gap between the artifacts and the code as assessed after Phase 8. Measured, not
inferred — every task below cites the evidence that produced it.

- [X] T035 **CRITICAL** — Fail closed on a wrapped scenario whose continuation opens with a keyword, instead of emitting a mis-assigned or empty clause, per FR-022 (contradicts). Shape: a scenario wrapped immediately before `**Then**` (the most common wrap in this repository) — §3 condition 4 correctly refuses to join the continuation, so the logical line carries only `Given`+`When`, T0 never fires, and L1 absorbs the When clause's text **plus a literal `**When**` keyword** into the `given` bucket; the `Then` line then arrives, matches L3, and flushes a scenario whose `when` is empty. Concrete instance: `specs/023-advance-board-position/spec.md:193-194`.
  **Implementation found a second, more severe face during the fix, invalidating the plan's original remedy** — a bare-keyword-in-body check (regex `(^| )(Given|When|Then)( |$)`) was prototyped first and rejected: the swallowed keyword retains its `**`/`__` wrapper in the raw captured text (`markdown_tokenize_inline` strips it only at tokenization, after the check would run), so the regex never caught a real instance, and on the un-wrapped form it produced a **false positive** on the already-pinned contract §4 row 9 (`Given a When b When c Then d`, where `given`'s correct text is literally `"a When b"`). Dropped per Constitution XIV/XV rather than kept as inert code.
  The **second real defect**, found while tracing why the rejected check still passed some corpus files: when scenario N's own Then is swallowed this way (given/when populated, then never separately reached), the flush guard on a new `**Given**` line checked only `then_items` — so an unrelated **following** scenario's Given/When silently appended onto scenario N's stale arrays instead of starting fresh, **merging two authored scenarios into one**. Concrete instance: `specs/007-fix-unicode-config-keys/spec.md:125-129` (items 1 and 2 collapse into a single corrupted scenario). This directly violates US4 AC3 ("two wrapped scenarios in sequence... remain two scenarios").
  **Actual fix, both ports**: (1) `_parse_ac_flush` emits only when `then` **and** `given` **and** `when` are all non-empty; (2) the Given-opener branch flushes *unconditionally* whenever any of given/when/then already holds state, not only when `then` is non-empty — discarding an incomplete prior scenario instead of merging into it. No keyword-body regex needed; both real defects are fully covered by the empty-bucket check alone. **Measured over `specs/*/spec.md`, before → after this fix: scenarios with an empty `given`/`when` 59 → 0; scenarios with a merged multi-clause `given`/`when` (3 files: 007, 017, 019) → 0; total emitted scenarios 554 → 492 (the 62 fewer are exactly the malformed ones now correctly refused rather than corrupted).** Both ports verified identical on the 023 and 007 shapes. Regression tests added in both ports using the real 023 and 007 source lines, plus a control asserting a normal multi-scenario document is unaffected by the unconditional flush. Full suites re-run clean: 53/53 bash, 39/39 Pester.
- [X] T036 Add an **unadorned** (no-emphasis) acceptance scenario to `tests/conformance/fixtures/repo-with-template-form-ac/specs/001-template-form/spec.md` per FR-015 (missing). FR-015 requires the corpus to carry "the emphasised single-line form, the emphasised wrapped form **and the unadorned form** explicitly"; T015/T023 added the first two, the third is absent. Evidence: all 355 Gherkin lines across the corpus's 49 fixtures carry `**`/`__` — **zero unadorned lines exist anywhere in the corpus**, so the form the spec calls "the currently correct path" (US1/AC3) has no cross-port proof. Re-run `bash tests/conformance/ci-conformance.sh` and update the recorded scenario count in `tests/bash/ci/test_conformance_no_cross_os_shard.bats` only if the scenario count itself changes. **Added as User Story 3 in the fixture; both ports verified parsing it identically before the corpus run; scenario file count unchanged so the recorded 190 stands. `ci-conformance.sh`: exit 0, temp-path noise only.**
- [X] T037 Add an input that **yields no scenario** to the same conformance fixture per FR-016 / SC-006 / US3 AC3 (missing). SC-006 requires byte-identical output "including for inputs that yield no scenario" and US3 AC3 makes it an acceptance scenario in its own right, but the fail-closed (T3) and FR-022 shapes are pinned by mirrored **unit** tests only — the corpus, which is the project's cross-port proof mechanism, never sees one. Use the contract §2 T3 shape (three keywords, no grammar the parser accepts) and, once T035 lands, the FR-022 clause-boundary wrap. Depends on T035 for the second shape. **Added as User Story 4 (reversed-keyword-order T3 shape). Verified on both ports: the story is created with no `acceptance_criteria` field, identically. The FR-022 clause-boundary-wrap shape was not additionally added here — after T035, that shape now correctly emits nothing via the SAME empty-bucket mechanism the T3 shape already exercises, so it would not add cross-port proof coverage beyond what T3 already gives; T035's own regression tests (both ports) already pin the FR-022 shape directly at the unit level.**
- [X] T038 [P] Add single-marker emphasis cases (`*Given*` and `_Given_`) to `tests/bash/engine/test_parse_title_desc.bats` and `tests/powershell/engine/Parse.TitleDesc.Tests.ps1` per US1 AC4 (partial). The wrapper token accepts `**`, `__`, `*` and `_`; only `**` and `__` are exercised by any test. Both single-marker styles were verified working by hand during convergence, so this is a coverage gap rather than a defect — but US1 AC4 ("any emphasis style the parser already accepts elsewhere ... is read exactly as the bold form is") is unverified, and Constitution XIII's "every behaviour originates from a failing test" leaves it free to regress silently. **Added and green on both ports without any production change, confirming the coverage gap, not a defect.**

**Phase 9 closing verification**: `shellcheck -x -P scripts/bash` clean; PSScriptAnalyzer with the repo's `PSScriptAnalyzerSettings.psd1` clean; `git status` scope re-confirmed (only `parse.sh`, `Parse.psm1`, tests, the fixture, the two scenario files, `tasks.md`, `CHANGELOG.md` — no sink file); both `engine-sink-boundary` gates re-run locally, 0 violations. Full suites re-run after all of Phase 9's changes: `tests/run-bash.sh` 249 files / 2302 tests, 0 failed; full Pester 1773/1773 passed. CHANGELOG amended with T035's finding. The `ci/windows-probe` push from before Phase 9 (commit `a995afe`) predates the T035 fix and needs re-pushing to prove the final state on the real runner — tracked under T033, still held for operator go-ahead.
