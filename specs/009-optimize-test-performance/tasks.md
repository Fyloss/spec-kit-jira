...
- [X] T013 [P] [US1] Update the "running the tests" documentation (`README.md` and/or `INSTALL.md`) to state the fast local path (`tests/run-bash.sh`, only `bats`+`jq` required, no PowerShell/GNU `parallel` for the Bash suite) per FR-014 / Constitution XVI.
...
- [X] T020 [P] [US3] Verify parity detection is unimpaired (SC-010): temporarily inject a one-line divergence into a Bash-port output path in a scratch copy, run a conformance scenario against both ports, confirm the `diff` FAILS, then revert. Document the check in `quickstart.md` V6 (already drafted) as executed.
...
- [X] T022 [P] [US3] Verify zero tests were removed (SC-007/FR-004): compare the current `@test` count and file list to the T001 baseline; the executed-assertion count must be **≥ 955 across ≥ 115 `.bats` files** (the post-008 baseline — NOT the superseded pre-008 figure of 775/95). Record in `baseline.md`.
...
- [X] T024 Run the full `quickstart.md` validation (V1–V7) and record pass/fail for each scenario in `baseline.md`.
...
- [X] T026 [P] Add a `CHANGELOG.md` entry (Constitution XII / SemVer) describing the test-performance optimization: `curl` shim backend, dependency-free runner, CI caching; note the removed PowerShell/GNU-`parallel` requirement for the Bash suite.
- [X] T027 Final Constitution re-check: confirm no production code under `scripts/**` changed (`git diff --name-only main -- scripts/` is empty), all gates green three-OS, and every FR/SC in `spec.md` is satisfied; note any deviation for review.
...
**Purpose**: Close the gaps between the artifacts' stated intent and the current
tree, found by assessing the codebase against `spec.md`, `plan.md`, `tasks.md`,
and the constitution. Appended by `/speckit-converge`; ordered CRITICAL/HIGH
first.

- [X] T028 **CRITICAL** — Resolve the PID-keyed cursor race that this feature's parallelism now surfaces on a blocking gate, per FR-012 / SC-009 / Constitution XIII (contradicts). `scripts/bash/engine/story_marker.sh:32` keys the `SPEC_KIT_JIRA_ID_SOURCE` cursor as `${TMPDIR:-/tmp}/.speckit-jira-id-index.$$` — PID alone, in a shared TMPDIR — so a reused PID inherits a stale cursor from an unrelated dead process. `baseline.md` records one of two full-corpus runs diverging on `us2-parent-second-run` from exactly this. FR-012 forbids optimizations that turn a deterministic test intermittent, and the affected gate is conformance. Per Constitution X/XIII write the failing regression test FIRST (a concurrency reproduction under `tests/bash/engine/`), then either fix the key (e.g. PID + a per-run nonce recorded by the caller) or, if the plan's "`scripts/**` untouched" constraint is held, record an explicit reviewed deviation with a tracked follow-up issue. Silently leaving it undocumented is not an outcome.
- [X] T029 Rewrite the "Running the suites locally" section of `docs/09-ports-and-quality.md` (lines ~172–181) per FR-014 / FR-003 / Constitution XVI (contradicts). It currently instructs `bats -r tests/bash --jobs "$(getconf _NPROCESSORS_ONLN)"` and explains that `--jobs` parallelises "via GNU parallel" — the precise command FR-003/SC-003 identify as executing zero tests and reporting a false green on a host without GNU `parallel`. Replace with `tests/run-bash.sh`, state that only `bats` and `jq` are required, and mention `--since <ref>` for the inner loop.
- [X] T030 Add the scheduled CI job that runs `tests/run-bash.sh` **20 consecutive times** and reports, per SC-009 / quickstart V7 (missing). No workflow currently carries a `schedule:` trigger for the Bash suite — only `live.yml` schedules anything. SC-009's strong evidence deliberately lives in CI rather than the inner loop, so without this job half the criterion is unmet. The job MUST be non-blocking on pull requests (it is evidence, not a gate) and must not alter the 9-job blocking inventory (SC-006/FR-010).
- [X] T031 Add dependency caching to `.github/workflows/gates.yml` per FR-008 (partial). T016 was applied to `ci.yml` only — `actions/cache` appears nowhere in `gates.yml`, so `coverage-bash` re-downloads and rebuilds the pinned bats-core v1.13.0 tarball plus apt `kcov` on every run, and `coverage-pwsh` reinstalls Pester from PSGallery on every run. Cache each keyed on OS + the pinned version already in the file; on a miss, install exactly as today (correct, slower). A cache hit MUST NOT be able to change a gate verdict — do not cache the coverage report or any measured artifact.
- [ ] T032 Complete T019's verification of the coverage floor and denominator per FR-005 / SC-008 (partial). `baseline.md` records both as "TBD (needs Linux + kcov)"; the mock backend was swapped underneath the coverage exercise and neither number has been re-measured since. Read the percentage and the measured line count off a real `coverage-bash` job run, confirm ≥80% and a denominator ≥ the pre-change value, and record both plus the run URL in `baseline.md`. Never widen `kcov-excl` or shrink the denominator to reach the floor.
- [X] T033 Update `AGENTS.md`'s "Running the suites" section per FR-014 (partial). Line 26 still names `bats -r tests/bash` at "~15 min" as the way to run the full Bash suite; the fast path is now `tests/run-bash.sh` (measured at 190s, `bats`+`jq` only). Keep the `-r` warning for anyone who still invokes `bats` directly.
- [X] T034 Close T017c explicitly, per plan Decision 7 / SC-005 / FR-018 (missing). `ci.yml`'s conformance step runs all 51 scenarios serially on each of three OSes, unsharded; `baseline.md` documents a considered deferral (a shard-per-runner shape would add job names that repo-external branch-protection config would not mark required, risking a silently non-blocking corpus — the SC-006 hazard). Either implement in-OS sharding together with the branch-protection update, or convert the deferral into a recorded, reviewed scope decision and mark T017c closed-as-deferred. `test_conformance_no_cross_os_shard.bats` stays the guard either way: shards must never span OSes and every host must still run all 51.
- [ ] T035 Complete T023b's Windows verification per SC-011 / FR-019 / FR-020 (partial). The static guard passes, but SC-011 requires the per-OS scenario counts as evidence and `baseline.md` records V3c as "not directly verifiable from this machine". From a real `unit` run, confirm the `windows-latest` leg executed the full Pester suite AND all 51 conformance scenarios on that host, that both are blocking, and record the per-OS counts plus the run URL in `baseline.md`. Any host reporting fewer than 51 is a FAIL, not a speed-up.
- [ ] T036 Complete T025's CI measurement per SC-004 / SC-005 / SC-005b (partial). `baseline.md` still records CI runner-minutes and wall-clock as "TBD (needs a real Actions run)". Measure `coverage-bash` ≤ 15 min, push → complete merge decision (all 9 blocking jobs reported) ≤ 20 min, and total runner-minutes below a green run of `21068d6`. Record all three with run URLs in `baseline.md`; a budget miss is a failure of this feature, not a rounding note.
- [X] T037 Resolve T001's missing wall-clock denominator per SC-001 (partial). `baseline.md` records both the serial and parallel pre-change baselines as unmeasurable on the capture machine (no `pwsh`, no GNU `parallel`), which leaves the "SC-001 Target (like-for-like)" section pointing at a comparison that was never run. Either capture baseline (b) on a Linux/CI host at `21068d6`, or mark that section formally withdrawn in favour of the absolute budget — do not leave it reading as an open comparison.
- [X] T038 Record the `boundary.yml` review outcome in `baseline.md` per FR-009 / T018 (partial). The file is checkout plus two greps with no toolchain install, so no caching or targeted-install applies — but T018 requires that "nothing applies" result to be written down, and `baseline.md`'s FR-009 Scope Decision section never mentions the workflow.
- [X] T039 Align `quickstart.md` V1's pass condition with SC-002's scope note (partial). V1 (line ~34) still demands "0 skipped/failed for missing tooling", while spec.md's SC-002 explicitly places pwsh-side assertions of a shared harness out of scope — forcing `baseline.md` to record V1 as PASS-with-a-caveat. Restate V1 as "0 tests skipped or failed for missing tooling **among the Bash port's own tests**", so a future reader cannot read the caveat as an unmet criterion.
- [X] T040 **CRITICAL** — Fix the Linux-only failures the first real PR CI run exposed (static-checks `Bash port` step and the ubuntu `unit` leg red at `789885c`; macOS green), per FR-012 / SC-003 / Constitution X. Three distinct defects, all reproduced then re-verified green in an ubuntu-24.04 container carrying Debian's bats 1.10: (1) the runner meta-tests' fixture heredocs — bats 1.10 preprocesses column-0 `@test` lines inside the outer file's heredocs, so the fixtures reached the inner bats with no tests left (`1..0`); fixtures are now assembled at run time (`tests/bash/ci/test_run_bash_runner.bats`). (2) `NO_PARALLEL_PATH` dropped whole directories containing `parallel`, which on Debian images (bats Recommends GNU parallel into the merged `/usr/bin`) removed bash itself — exit 127; `parallel`-carrying directories are now shadowed, never removed. (3) an outer bats' injected libexec dir on PATH resolves to Debian's non-standalone libexec `bats` in the workers; `tests/run-bash.sh` workers now strip that dir and scrub inherited `BATS_*` state, with a regression test simulating the Debian split on any host (written first, observed red).
- [X] T041 Route the `coverage-bash` gate's traced suite and its suite-green rescue re-run through `tests/run-bash.sh` (parallel) instead of serial `bats -r` (`tests/coverage/bash-coverage.sh`, `.github/workflows/gates.yml`), per FR-016/SC-004: the serial form had outgrown every step timeout since the suite passed ~900 tests — `coverage-bash` has been red on `main` since `1044db9` (2026-07-28), killed at its 30-minute ceiling on every run. The 15-minute budget stays as SC-004 declares it; whether the parallelised phases now fit inside it is exactly T036's measurement.
