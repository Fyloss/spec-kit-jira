...
**Purpose**: Work the spec, plan and constitution call for that the codebase does not yet
satisfy. Established by running the suites, not by reading Phase 1–8 checkboxes: the Bash
unit suite passes 931/931, the PowerShell unit suite passes 709/709, and the 50-scenario
conformance corpus is byte-identical across both ports. The behaviour of US1–US5 is real
and green; the items below sit around it — two blocking quality gates the constitution
names and this repository does not run, one requirement whose deletion landed in one
fixture out of seven, and three coverage gaps whose fixtures or documentation exist but
are never reached.

- [X] T105 CRITICAL — Add a blocking lint job to CI that runs `shellcheck -x` over `scripts/bash/**/*.sh` with the repository's `.shellcheckrc` and `Invoke-ScriptAnalyzer` over `scripts/powershell` with `PSScriptAnalyzerSettings.psd1`, failing the build on any finding, in `.github/workflows/ci.yml` (or a sibling workflow). `ci.yml` installs `shellcheck` on the Linux and macOS runners today and never invokes it, and PSScriptAnalyzer is installed nowhere, so neither configuration file is enforced by anything per Constitution XII (missing)
- [X] T106 CRITICAL — Clear both ports to zero lint findings so T105's gate passes: `shellcheck -x` currently exits 1 with SC2178 and SC2128 at `scripts/bash/engine/story_marker.sh:287,297` (a variable used as an array then assigned a string) and SC2318 and SC2250 at `scripts/bash/lib/config.sh:232,244`; `Invoke-ScriptAnalyzer` returns 12 Warning-severity findings across `scripts/powershell` (7× PSUseSingularNouns, 3× PSAvoidUsingEmptyCatchBlock, 1× PSAvoidAssignmentToAutomaticVariable, 1× PSUseDeclaredVarsMoreThanAssignments). Fix, or add a justified per-site suppression, per Constitution XII (contradicts)
- [X] T107 Delete the stray `projects[].issue_types` logical-name-to-identifier map from the six committed fixture configurations that still declare it — `tests/conformance/fixtures/{repo-with-config,repo-with-core,repo-with-mirrored-spec,repo-with-parent-creating,repo-with-readme,repo-with-unicode-binding}/.specify/jira/config.yml` — per FR-030a. T031 deleted it from `repo-with-reconcile-binding` only, because the spec assumed a single fixture declared it; seven did. `repo-with-parent-creating` is a fixture **this feature added** carrying the shape FR-030a deletes, so the map is being introduced as fast as it is removed. Identifiers belong in the gitignored binding (Constitution V) (missing)
- [X] T108 Add the conformance scenario `tests/conformance/scenarios/us1-hierarchy-nonlatin.json` driving `tests/conformance/fixtures/repo-with-nonlatin-project/` (T007) through a full reconcile, asserting every creation carries that fixture's own type identifiers, per US1's Independent Test and SC-004. The fixture and `mock-jira/configs/nonlatin.json` are today referenced by no scenario and no test, so CJK/Cyrillic type names are exercised only by `tests/bash/sink/test_hierarchy.bats:132` and its Pester twin — two independent files that never diff each other's bytes. Spec Constitution Check VI requires both ports to emit non-ASCII type names in the same encoding across the three-OS matrix, Windows PowerShell output included, which only a conformance scenario proves. Sits beside `us1-hierarchy-french.json` and `us1-hierarchy-safe.json`, which the same Independent Test names as the other two of its three fixture projects (partial)
- [X] T109 Reconcile a child's parent reference when the child already exists but names the wrong parent, counting it as a write and leaving an unchanged second run at zero writes, in `scripts/bash/sink/jira/plan_apply.sh`, `scripts/powershell/sink/jira/PlanApply.psm1` and their recognition inputs. The UPDATE branch states "the parent link is never re-touched (it was set at creation)" (`plan_apply.sh:121`) and nothing reads a recognised child's current parent, so the spec's edge case is unimplemented. Note the boundary the spec draws: Out of Scope excludes **migrating pre-feature flat mirrors**, which is what the CHANGELOG's Migration note documents; a child linked to the *wrong* parent is a different case and is in scope. Write the failing test first, per the repository's bug-fix policy (missing)
- [X] T110 Add the live-integration workflow that runs `tests/live/` on push to the default branch, on a schedule, and on a maintainer-applied label, per Constitution XII — never as a blocking gate on fork pull requests. T098/T099 extended `tests/live/test_live_zero_churn.bats` to cover the parent and the parent link, but no workflow invokes `tests/live/` at all, so SC-003's requirement that the zero-write proof be "verified against a real Jira instance and not only against a mock" has no mechanism (missing)
- [X] T111 Execute the thirteen steps of `specs/008-jira-parent-hierarchy/quickstart.md` end to end including the live dogfood run against the consuming project, and record the outcome as `specs/008-jira-parent-hierarchy/quickstart-results.md`, following the precedent `specs/002-config-discovery-team-prefix/` and `specs/003-install-hook-activation/` set. T104 asks for this evidence and no such record exists; Constitution XII names the dogfood record as a verifiable release gate (missing)
- [X] T112 Document the `binding-shape-stale` refusal for operators — the state every existing installation, the maintainer's machine included, is in on its first run after this release — in `INSTALL.md` (whose "Upgrading from a release that registered its own hooks" section is the natural home, and which currently carries zero mentions of the parent, the hierarchy, or the ceremony's new child-type question), in `commands/speckit.jira.reconcile.md`'s refusal table beside the "Not yet configured" row, and in the CHANGELOG's Migration note. FR-003a requires the refusal to be *tested* rather than release-noted, which it is; it does not excuse leaving the operator's remediation undocumented (Constitution XVI, T100) (partial)
- [X] T113 Assert end to end that a specification with no `## User Story` headings mirrors as one parent **and one child**, not a parent alone, in `tests/bash/commands/test_reconcile_hierarchy.bats` and `tests/powershell/commands/Reconcile.Hierarchy.Tests.ps1`. T050 pins the non-collision at the parser level (`test_spec_marker.bats:78`) — that the implicit story still gets its own `story=` marker — but nothing asserts the reconcile outcome the spec's edge case describes (partial)
- [X] T114 Make the conformance harness hermetic: scrub every ambient `SPEC_KIT_JIRA_*` / `JIRA_*` variable in `tests/conformance/run-scenario.sh` before applying the scenario's own `env`, exempting the harness's own configuration and routing a caller's deliberate one-run override through the named `SPEC_KIT_JIRA_HARNESS_ENV` channel; and clear the environment `tests/powershell/lib/TokenLeak.Tests.ps1` and `tests/powershell/commands/Feature.Tests.ps1` set but never released. `TokenLeak.Tests.ps1` exported `SPEC_KIT_JIRA_PROJECT_KEY=PROJ` — the shipped placeholder — for the rest of the Pester process; on hosts that discover `tests/powershell/lib` *after* `tests/powershell/commands` (whose `Reconcile.*` files scrub that variable) nothing noticed, and on the Linux CI host, which discovers it immediately before `tests/powershell/conformance`, all four reconcile-driving conformance tests (T039 ×2, T108, T041) refused with the placeholder-key message instead of mirroring. Green on every developer machine, red in CI, with nothing in either log naming the cause. Regression tests first, per the repository's bug-fix policy: `tests/bash/conformance/test_harness_isolation.bats` and `tests/powershell/conformance/HarnessIsolation.Tests.ps1` (defect)
