...
## Phase 7 — PR review feedback

Appended on 2026-09-02 from the automated review on the pull request. The review
reported one finding; sweeping for its class found three.

- [X] T115 Fix every stale path reference in this feature's own documents, and guard the class. `quickstart.md` named a `test_artifact_publication.bats`, a file that never existed under that name (the cases live in `test_attachments.bats`), and `tests/powershell/Engine/…`, whose capital `E` resolves only on a case-insensitive host — so an operator following the runbook on macOS would see `bats` and `Invoke-Pester` report success having run nothing, and on ubuntu would see them find nothing at all. The same sweep found a third, which the review missed: T044 above is marked `[X]` while citing a `test_comment_body.bats`, a file that does not exist — the body cases it describes are real, and sit in `test_attachments.bats` beside the publication decision they assert against. The PowerShell half of the runbook's unit snippet also named no sink test at all, leaving the collision, classification and withholding-order rows of its own table unprovable from the commands given; `Attachments.Tests.ps1` is now there. New guard: `test_doc_test_paths_exist.bats` under `tests/bash/ci/`, scoped to the feature named by `.specify/feature.json` and matching against `git ls-files` so the comparison is case-sensitive on the host where it is not. Proven RED against all three before the fix, and its second case plants a defect so the guard cannot go inert
