...
- [ ] T019 [P] [US2] Bats suite in tests/bash/sink/test_discovery_list_projects.bats: `discovery_list_projects` paginates `GET /project/search`, extracts `key`/`name`/style per the §1 mapping into a canonical array, and fails closed with the "no visible project" error on zero results
- [ ] T020 [P] [US2] Pester twin in tests/powershell/sink/Discovery.ListProjects.Tests.ps1 for `Get-JiraDiscoveryProjectList`
...
- [ ] T022 [P] [US2] Pester twin in tests/powershell/commands/Config.KeySources.Tests.ps1
...
- [ ] T024 [P] [US2] Pester twin in tests/powershell/commands/Config.Degraded.Tests.ps1
...
- [ ] T026 [P] [US2] Conformance scenario tests/conformance/scenarios/us2-list-projects.json: connected run with no key ⇒ paginated project/search call sequence recorded, closed-question error/output lists the accessible projects, byte-identical across ports
- [ ] T027 [P] [US2] Conformance scenario tests/conformance/scenarios/us2-placeholder-key-refusal.json: `config.yml` holding `PROJ`, arbitrary branch prefix checked out ⇒ exit 4 unattended, summary contains no branch-derived value (quickstart scenario 3)
- [ ] T028 [P] [US2] Conformance scenario tests/conformance/scenarios/us2-degraded-mode.json: no base URL in env ⇒ exit 0, provisional proposals, re-run guidance, zero writes, zero mock-Jira calls; both ports byte-identical
