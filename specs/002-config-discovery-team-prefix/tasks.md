...
- [X] T101 Failing twin guards + fix: the `bash-coverage` CI job died at `apt-get install -y kcov jq curl` with `E: Unable to locate package kcov` — kcov was dropped from the Ubuntu 24.04 (noble) archive that backs `ubuntu-latest`. Pinned the job to ubuntu-22.04 (jammy still ships it in universe), the same pin the Gates workflow's `coverage-bash` job already carried. Guarded by tests/bash/ci/test_workflow_kcov_runner.bats + tests/powershell/ci/Workflow.KcovRunner.Tests.ps1, which fail on any workflow job that apt-installs kcov on an image whose archive dropped it (a source build is exempt).
  - The job had never reached its measurement step, so T097 stays open: the step now runs `--mode conformance --threshold 80` against the ~55% floor documented above and may fail on the threshold rather than the install.
- [X] T102 Failing twin guards + fix: the first run that reached the measurement step produced no report and no diagnostics — GitHub killed it with `The action 'Measure statement coverage (Constitution XIII)' has timed out after 20 minutes`. Three defects, each now pinned by tests/bash/ci/test_coverage_runner_bounds.bats + tests/powershell/ci/Coverage.RunnerBounds.Tests.ps1 and by a behavioural test in tests/bash/conformance/test_run_scenario.bats:
  - **The hang.** `mock_start` launched the pwsh mock with the caller's descriptors, and `run-scenario.sh` reaped it only on the success path — a scenario that aborted after startup (`set -e`) orphaned it. An orphan holds every descriptor it inherited, so whoever reads the other end waits for an EOF that cannot arrive. Reproduced locally, without kcov: the new harness test first hung on bats' `run` pipe, then on bats' diagnostic fd 3, and only failed cleanly once both were closed. kcov is the same shape of reader — it swaps the traced program's fd 2 for a pipe and reads to EOF before writing a report. Fix: the mock inherits nothing (`< /dev/null`, stdout/stderr to files under `MOCK_TMPDIR`, `3>&-`), and the harness installs `trap mock_stop EXIT` the moment the mock is up.
  - **The silence.** Nothing bounded the run, so the runner's own 20-minute wall was the first thing to fire and it took the diagnostics with it. `bash-coverage.sh` now runs kcov under `run_bounded` (`SPEC_KIT_JIRA_COVERAGE_TIMEOUT`, default 600s; `timeout`/`gtimeout` when present, unbounded when neither is), prints the exercise's `progress.log` afterwards, and the CI step budget sits above that bound instead of below it.
  - **The measurement.** Every exercise phase ran under `> /dev/null 2>&1`. kcov collects the trace on fd 2, so that redirection discarded every traced line: had the run finished, it could only have reported "no coverage produced". Phases now redirect stdout only and read stdin from `/dev/null` (a phase reaching a prompt would otherwise block on the runner's stdin). The port's stderr consequently reaches the job log, which is the documented cost of measuring at all.
  - T097 still stays open on the threshold itself: nothing here raises coverage, it only lets the number be produced and read.
- [X] T103 Failing twin guards + fix: the first run that produced a number reported **59.24% (1276/2154 lines)** and failed the gate. The number was real, but it measured neither the right suites nor the right denominator. Pinned by tests/bash/ci/test_coverage_bats_measurement.bats + tests/powershell/ci/Coverage.BatsMeasurement.Tests.ps1 (shape) and by behavioural tests of the extraction and the merge, which run anywhere:
  - **The wrong suites.** Constitution XIII computes coverage on the mocked unit suites, but kcov cannot run bats — it instruments bats-core's own DEBUG-trap tracing, and the two feed each other a trace that never ends (91.7 GB observed). CI therefore measured only the conformance corpus, which drives happy paths and encoded refusals and never reaches the error branches `lib/credentials.sh` (12.50%), `engine/drift.sh` (14.89%), `engine/interchange.sh` (18.07%) and `sink/jira/client.sh` (20.00%) are unit-tested for. The runner now collects the bats suite itself, with `PS4` carrying `file:line` and `BASH_XTRACEFD` pointing at fd 8 — a channel nothing in bats-core reads or re-prints, so there is no feedback loop and no kcov. `SHELLOPTS=xtrace` is exported, so the trace follows execve'd children, which kcov never did.
  - **The wrong denominator.** The port already brackets its multi-line jq literals with `# kcov-excl-start` / `# kcov-excl-stop` — 533 lines whose continuation kcov's line parser counts as statements that no execution can ever hit, because the whole literal traces as ONE statement at its opening line. `bash-coverage.sh` never passed `--exclude-region` (the Gates workflow's job did), which alone capped `engine/drift.sh` at 17% no matter what the tests do. Verified locally against a bash-3.2 sample: 50% → 80% with the flag.
  - **The merge.** kcov stays the tool Constitution XIII names and stays the authority on what counts as a statement: the gate reads its per-line `cobertura.xml` (`coverage.json` carries only per-file totals) and marks a line covered when either exercise ran it. A traced line kcov does not instrument cannot inflate the score. `--mode full` (kcov + traced bats, the CI gate), `--mode conformance` (kcov alone), `--mode bats` (the trace alone — hit counts, no denominator, no gate: the one mode that runs on macOS, where kcov cannot drive this port at all).
  - **The tracer must not perturb the suite.** `PS4` expands in every traced shell, including the `bash -c` children the suite runs under `set -u`; `${BASH_SOURCE}` without a default made those children print `BASH_SOURCE: unbound variable` on their own stderr, which landed in the output bats captured and turned 28 green tests red. With `${BASH_SOURCE:-}` the whole suite passes under tracing (455/455).
  - **Measured locally** (macOS, bats 1.13, no kcov): the bats suite alone hits 1701 of the 2154 lines kcov currently counts — 78.97% against the *inflated* denominator, and the conformance corpus adds essentially nothing on top of it (the union is the same 1701). With the excl regions honoured the denominator drops by roughly 360 lines, which puts the merged figure comfortably over the gate. The exact merged number can only be observed on Linux CI: kcov cannot run this port on macOS at all.
