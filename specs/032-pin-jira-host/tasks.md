...
- [X] T031 [US1] Add multi-run conformance scenario `tests/conformance/scenarios/us032-accept-site-replay.json` for SC-008 (C3.9). **Done — the obstacle recorded here earlier no longer exists.** It was real when written: the ceremony refused a changed destination at its WRITE site, after discovery, so the redirected host had to be reachable, and the two mock backends (a curl shim on Bash, a real socket on PowerShell) disagree about an unreachable host. Moving the ceremony's check AHEAD of discovery removed it — the redirected host is now never contacted, so the backends cannot disagree. Measured on both ports: run1=0 (binds), run2=4 with zero calls (reconcile refuses), run3=4 with zero calls (the ceremony refuses the replayed instruction). CI ran the full 243-scenario corpus green on ubuntu, macos and windows.
  **Still per-port, and stated rather than hidden:** the POSITIVE half — accepting by name — reaches discovery and so remains backend-sensitive. It is covered by 9 cases each in `test_config_bound_site.bats` / `Config.BoundSite.Tests.ps1`, not byte-compared.
- [X] T032 [US1] Add fixture `tests/conformance/fixtures/repo-032-accept-site-replay/`, tracked. Done — 5 files in git, verified by the fixture-tracking guard.
...
- [X] T078 Run `tests/run-bash.sh` (full) and the Pester suites; both green. **Verdict recorded from CI, which is where both suites actually completed.** The `CI` workflow is green 17/17 on commit 52cb71a: unit suites (bash + Pester) on ubuntu, macos AND windows, and the 243-scenario conformance corpus on all three. Locally the bash suite is green (263 files / 2599 tests / 0 failures); the full Pester suite was killed six times by memory pressure on this host and never completed here — CI is its first and only complete run, which is stated in the feature commit message rather than glossed over.
  One `windows-latest` conformance shard failed once with *"the hosted runner lost communication with the server"* — an infrastructure fault, not a divergence — and passed on a single rerun, per the project's one-retry rule.
  `Gates` is 5/6: `Bash coverage >= 80% (kcov)` exceeded its 7200s internal ceiling (`gates.yml:159`) while the job itself allows 350 minutes. That is a tracing budget, not a red suite — see T082.
...
Appended by `/speckit-converge`. Neither item is unimplemented feature work —
the codebase satisfies all 17 functional requirements in both ports and all 8
success criteria have named test artifacts. Both are bookkeeping corrections.

- [X] T081 Correct the stale BLOCKED notes on T031/T032 in `specs/032-pin-jira-host/tasks.md` per `tasks.md` T031/T032 (contradicts) — the notes describe an obstacle that the pre-discovery ceremony check removed. The artifacts exist and are tracked (`tests/conformance/scenarios/us032-accept-site-replay.json`, `tests/conformance/fixtures/repo-032-accept-site-replay/`, 5 files in git); the 3-run scenario measures run1=0, run2=4 with zero calls, run3=4 with zero calls on both ports; and CI ran the full 243-scenario corpus green on ubuntu, macos and windows. What remains genuinely per-port is only the POSITIVE half — accepting by name still reaches discovery — and that limitation should be stated in one line rather than as a blocked task.
- [X] T082 Record the CI verdict for T078 per `tasks.md` T078 (partial) — done in T078 above. **The kcov ceiling is left as an open decision, deliberately not raised here.** Raising `SPEC_KIT_JIRA_COVERAGE_BATS_TIMEOUT` from 7200s would make the gate pass — the job allows 350 minutes, so there is room — but that job already costs roughly 3 hours per push, which was a known open question about its design BEFORE this feature added ~30 tests and 7 scenarios. Raising the ceiling each time it is exceeded postpones the design decision rather than taking it, and that decision is the maintainer's, not this feature's. Recorded for the reviewer instead of quietly patched.
