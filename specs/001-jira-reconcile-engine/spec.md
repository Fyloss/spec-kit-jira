...
- **NEEDS OPERATOR CONFIRMATION: command name** — `config` under-describes a command that also registers hooks and edits the consuming README; `setup` was the prior name. FR-054 shows the command's scope is discovery **plus** hook registration **plus** README block management, not configuration-file production alone; a reader would not expect a name echoing `git config` to register lifecycle hooks or edit the repository README. The operator MUST confirm the command name before `/speckit.plan`; this item is intentionally left unresolved here rather than renamed on the specifier's initiative.
