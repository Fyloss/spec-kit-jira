...
- [X] T079 [US4] US4 AC2 — a store that cannot unlock without a human is a command that blocks until the bound, which is exactly `us030-cred-command-timeout.json` (T048a). **Do not author a second scenario**: no test can stage a genuinely locked vault on three platforms, and a second one staged with the same hang command would assert the same bytes twice. Instead, extend T048a's description to record that it discharges US4 AC2, and verify its message names the command and the bound rather than "no token configured"
  Completion note (2026-08-18): `us030-cred-command-timeout.json`'s description already records the US4 AC2 discharge ("This fourth declared-failure path... also discharges US4 AC2"). Verified the message via the quickstart walk: `config: credential resolution failed: JIRA_PAT_COMMAND 'sleep 30' exceeded the 5s bound...` — names the command and the bound, never "no token configured".
...
- [ ] T086 **Coverage gate.** Confirm ≥80% statement coverage on every changed module in both ports (Constitution XIII); `scripts/bash/lib/credentials.sh` carries `kcov-excl` regions around the xtrace-suspended token paths, so check the exclusions still match the rewritten code rather than hiding new lines
  Note (2026-08-18): left unchecked, not deferred silently. A local `bash-coverage.sh --mode bats` run stalled for 1+ hour with zero progress past "Tracing the bats suite (fd 8)..." and was killed. The PR's own CI coverage gate (`gates.yml`, ubuntu-latest, kcov) hit the same wall — its kcov step timed out at its hard 15-minute limit, which reads as "coverage gate failed genuinely" in the check-run annotation even though no coverage percentage was ever measured (the fallback path only rescues an explicit "could not measure" exit, not a timeout kill — see `gates.yml`'s own comment on the `kcov` step). This is a real, unresolved kcov-performance problem against the current codebase size, reproduced independently on two different machines; it is not something this task's own scope (checking `kcov-excl` region placement) can fix, and it blocks ever getting an actual coverage number to confirm against the 80% threshold. Needs its own investigation.
...
- [X] T101 Run `shellcheck -x -P scripts/bash` over `scripts/bash/` only (a whole-tree scan is ~1900 lines of host-script noise) and `actionlint` over `.github/workflows/`; both clean
  Completion note (2026-08-18): both exit 0, zero findings. Update: the PR's own CI Lint job also runs PSScriptAnalyzer over scripts/powershell/ (not part of this task's own local command, and not covered by the local shellcheck/actionlint pass), and it caught two real findings this task missed — `Credentials.psm1`'s `Kill()`-after-timeout empty catch block, and `Config.psm1`'s `$CredReason` parameter tripping a plain-text-password name heuristic. Both fixed with a justified `SuppressMessageAttribute`, matching this codebase's existing convention. Same CI run's `windows-latest` unit-suite job also caught a real, unrelated defect this task's local (macOS) run could not: three `Credentials.Tests.ps1` cases (C2.3, C2.4, C4.4) wrote a raw `#!/usr/bin/env bash` shebang script and pointed `JIRA_PAT_COMMAND` at it directly, which doesn't execute on native Windows — fixed by adding the same `$IsWindows` branch the `C3.6` test right above them already had. A 4th windows-latest failure (`Reconcile.Target.Tests.ps1`) has zero diff from this feature's own commits and its own logic asserts no network request happens before the refusal it's testing, so it's left as the pre-existing Windows CI baseline, not a 030 regression.
...
- [X] T103 Push to `ci/windows-probe` (`.github/workflows/windows-conformance.yml`) and read the check-run **annotations** (~11 min; job logs 403 with the current token). Compare against the known-red baseline on `main`, not against green. One retry maximum, then hand the result back
  Completion note (2026-08-18): pushed (commit c5f477b), run took ~50 min (well over the usual ~11 min — real GHA windows-latest variance, not a hang; all 4 shards genuinely progressed throughout). Overall conclusion: failure. Read via check-run annotations (job logs still 403 with this token, per prior sessions' finding). Zero `us030-*` scenario names appear among the failures — every divergence is in pre-existing, unrelated scenarios (us021/us022/us023/us027), matching the documented known-red baseline (`bash=0a pwsh=7b`-shaped: bash stdout empty, pwsh's is a full JSON document) rather than a 030-introduced regression. No retry taken — the result was clear on the first push.
- [X] T104 Verify no task in `specs/030-retire-env-credentials/tasks.md` is checked without its artifact present in the tree — on spec 029 three of four converge findings traced to checked tasks whose deliverable did not exist
  Completion note (2026-08-18): dispatched as a full sweep against commit c5f477b. One real gap found: T063a claimed two files were edited that weren't — resolved by documenting, under T063a itself, that the substance it describes is actually covered by `test_config_degraded.bats`/`Config.Degraded.Tests.ps1` (T062's own test), not the files T063a names. Every other phase's checked tasks were spot-verified against their actual artifacts and found clean. This sweep also surfaced that T079/T103/T104 itself were substantively done but left unchecked — fixed alongside this entry.

- [X] T105 Address the Copilot review comments on the PR — two findings, one refuted and one real.
  Completion note (2026-08-18): finding 1 (`tests/powershell/lib/PersonalConfig.Tests.ps1`, "extra closing brace ends the `Describe` early") is a **false positive** — the file parses clean (`[Parser]::ParseFile`) and all 15 `It` cases run; the brace closes the `It` above it and was merely indented to column 0, which is what misread as a `Describe` terminator. Reindented to `    }` and dropped the stray whitespace-only line; no semantic change, 15/15 still pass.
  Finding 2 (`scripts/powershell/lib/Credentials.psm1`, bound-exceeded path) is **real, but not the leak it was reported as**. Measured on macOS before fixing: the killed child is reaped by .NET's own SIGCHLD handler (0/15 runs left it visible to `ps`), so no process lingers; what does leak is **two pipe descriptors per timed-out attempt**, held until the finalizer happens to run. Fixed by wrapping the whole `Start`-to-`ExitCode` span in a `try/finally` that disposes the two `StreamReader`s **and** the `Process` — `$proc.Dispose()` alone does not close the redirected readers, measured, not assumed. On the timeout path the child is now reaped and both reads settled before returning, mirroring the Bash port's `wait "${pid}"`.
  Deviation from the suggested patch, deliberate: every post-`Kill()` wait is **bounded** (`WaitForExit(1000)`, `Task.Wait(500)`), never the argument-less `WaitForExit()` or the unbounded `GetAwaiter().GetResult()` the suggestion proposed. Both of those block until the pipes close, which a grandchild the command left behind holds open indefinitely — trading a deferred descriptor for a hung run.
  Failing test first, per the repo's bug-fix policy: `Credentials.Tests.ps1` gained a `Child-process hygiene on the bound-exceeded path` context — the descriptor test reads **+10 (FAIL) against the pre-fix module and 0 after** (verified by stashing the module fix and re-running), and a second test pins the no-hang property with a stub that backgrounds a `sleep` inheriting stdout. POSIX-only (`-Skip:$IsWindows`): there is no comparable per-process descriptor count on Windows. Port-local — the Bash port redirects to files it removes and reaps with `wait`, so no conformance scenario changes and no message text moved.
  Verification: `Credentials.Tests.ps1` + `TokenLeak.Tests.ps1` + `PersonalConfig.Tests.ps1` 49/49 green; `Invoke-ScriptAnalyzer -Settings PSScriptAnalyzerSettings.psd1` clean on the changed module (its existing `PSAvoidUsingEmptyCatchBlock` suppression justification was widened to cover the two new drain catches).
