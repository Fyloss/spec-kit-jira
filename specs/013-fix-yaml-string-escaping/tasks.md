...
- [ ] T058 **CRITICAL** Resolve — or record an explicit, justified scope-out decision for — the Windows conformance divergence `us2-field-defaults-question` (`stdout: first difference at byte 311 — bash=43 pwsh=2f`, sizes 434 / 415), then re-run `ci/windows-probe` to green per FR-023, SC-006, Constitution VI (contradicts). Probe run `30855631505` on HEAD `4f11ac4` exits 123 with that divergence, yet T049 is marked complete claiming zero divergences. The identical annotation appears on the pre-013 commit `64bff1b`, so the defect is **pre-existing rather than introduced by 013** — but Constitution VI's three-runner claim is undemonstrated either way, and AGENTS.md holds that a platform claim is unproven without a green probe run
