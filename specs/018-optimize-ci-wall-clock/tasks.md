...
- [ ] T011 Run the Windows probe (`git push --force origin HEAD:ci/windows-probe`) and record W1–W4 in `specs/018-optimize-ci-wall-clock/baseline.md` with the run id and commit — one retry maximum on an inconclusive run, then report as it stands (contracts/windows-probe.md, budget discipline)
