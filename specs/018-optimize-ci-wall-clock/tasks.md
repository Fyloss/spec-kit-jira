...
- [X] T019 [P] [US1] Write failing test for the reduced-spawn curl shim in `tests/bash/ci/test_mock_shim_contract.bats` (extend the existing file): identical responses, identical call-log order, and a bounded process count per request (R10, FR-003)
- [X] T020 [P] [US1] Write failing test for harness spawn reduction in `tests/bash/conformance/test_run_scenario.bats` (extend): `jq_lines` and the workdir snapshot produce identical captures with fewer process creations (FR-003)
...
- [X] T022 [P] [US1] Reduce test-owned spawns in `tests/conformance/mock-jira/curl-shim.sh` (measured: 27 `jq` per 8 HTTP calls) so T019 passes (D4)
- [X] T023 [P] [US1] Reduce test-owned spawns in `tests/conformance/run-scenario.sh` — the `jq_lines`+`sed` pair per scalar read, the `mktemp`/`cp` churn, the per-file snapshot loop — so T020 passes (D4)
...
- [X] T025 [US1] Apply the user's T015 decision to the Windows leg (option (a): install the LF-emitting `jq` in `.github/workflows/ci.yml` and the probe; option (b): the guard change, only with an explicit FR-020 exception recorded; neither: skip and proceed to T026) — **applied option (a) to `ci.yml` only, by the user's explicit condition (baseline.md); the probe stays on native `jq` deliberately, so this deviates from the literal "and the probe" wording**
...
- [ ] T032 [US2] **Runs before T031** — it produces the file T031 reads, so it is not parallel with it. Commit the per-file timing profile produced by T012 to `tests/bash-suite-timings.txt` (or the path T031 reads), documenting that it is a scheduling hint only — never a filter, and never a source of verdicts — **the capture-and-dump mechanism is built (`SPEC_KIT_JIRA_BATS_DUMP_TIMINGS` in `tests/run-bash.sh`, tested); the actual committed file still needs a real CI run's data, per research §4.2 — not produced from this local host**
- [ ] T033 [US2] Refresh the timing profile from the nightly in `.github/workflows/bash-suite-stability.yml`, so the ordering does not rot as the suite grows — **not parallel**: T042 edits the same workflow file (the 51→84 and 986→1427 growth is exactly what invalidated the previous budgets) — **not started: committing a profile FROM a scheduled nightly needs `contents: write` on a currently `contents: read` workflow — a permissions change flagged to the user rather than made silently, see baseline.md**
